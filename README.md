...
![Demo](example.gif)
Different waypoints (shown in purple in the gif) are evaluated based on their proximity to obstacles and whether they lie on the exploration frontier. If, during exploration, the algorithm detects a marker (shown in red), it immediately generates a waypoint directed toward the detection. This allows the robot to collect additional measurements of the marker’s position, leading to a more accurate final estimation.
